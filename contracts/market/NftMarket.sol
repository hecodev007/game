...
contract NFTMarket is Context, IERC721Receiver, ReentrancyGuard, InitializableOwner {
    ...
    function initialize(address payable tipsFeeWallet, address weth) public onlyOwner {
    ...
    returns (uint)
    {
        ...
        emit eveNewSales(obj.id, tokenId, msg.sender, nft, address(0x0), currency, startTime, 0);
        return _salesAmount;
    }
